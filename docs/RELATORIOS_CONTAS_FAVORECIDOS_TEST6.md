...
| Encerramento da conta | Controller enviava senha a parâmetro que esperava dados do usuário; a operação existente excluía registros. | Verificação da senha e identidade, proteção do último administrador e desativação transacional com invalidação de tokens. |
...
- `Database`: migração aditiva 4, `usuarios.Ativo`, padrão 1 para usuários existentes.
